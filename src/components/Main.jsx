...
const Main = ({ onToggleLightMode }) => {
  ...
  // Nav splash: only when navigating here (initial load is covered by index.html splash)
  const [navSplashVisible, setNavSplashVisible] = useState(() => !document.getElementById('cinestoke-splash'));
  const [navSplashExiting, setNavSplashExiting] = useState(false);
  const [navSplashMinPassed, setNavSplashMinPassed] = useState(false);

  useEffect(() => {
    if (!navSplashVisible) return;
    document.body.style.overflow = 'hidden';
    const timer = setTimeout(() => setNavSplashMinPassed(true), 2000);
    return () => { document.body.style.overflow = ''; clearTimeout(timer); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!navSplashVisible || !pageReady || !navSplashMinPassed) return;
    document.body.style.overflow = '';
    setNavSplashExiting(true);
    const t = setTimeout(() => setNavSplashVisible(false), 700);
    return () => clearTimeout(t);
  }, [navSplashVisible, pageReady, navSplashMinPassed]);
    ...
    <div className='main'>
      {navSplashVisible && (
        <div style={{
          ...
          opacity: navSplashExiting ? 0 : 1,
          transition: 'opacity 0.7s ease',
          pointerEvents: navSplashExiting ? 'none' : 'auto',
        }}>
          <svg width="140" height="44" viewBox="0 0 3000 940" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ animation: 'splashBob 2.4s ease-in-out infinite' }}>
            <path d="M1406.61 726.14C1442.21 763.34 1448.45 739.307 1447.11 722.64C1457.61 503.14 1305.61 449.14 1157.11 407.64C907.612 341.64 576.112 234.64 450.612 190.14C341.812 155.34 243.279 90.6398 207.613 62.6399C196.113 52.4732 175.013 39.6399 182.613 69.6399C210.113 158.14 207.113 174.14 365.113 274.64C601.613 382.14 1161.61 494.14 1214.61 506.14C1267.61 518.14 1305.61 570.64 1336.61 615.14C1350.11 658.14 1363.61 687.64 1406.61 726.14Z" fill="white"/>
            <path d="M1592.63 730.277C1557.03 767.477 1550.79 743.443 1552.13 726.777C1541.63 507.278 1693.63 453.278 1842.13 411.778C2091.63 345.778 2423.13 238.779 2548.63 194.279C2657.43 159.479 2755.96 94.7793 2791.63 66.7795C2803.13 56.6128 2824.23 43.7796 2816.63 73.7794C2789.13 162.279 2792.13 178.279 2634.13 278.779C2397.63 386.278 1837.63 498.278 1784.63 510.278C1731.63 522.278 1693.63 574.777 1662.63 619.277C1649.13 662.277 1635.63 691.777 1592.63 730.277Z" fill="white"/>
